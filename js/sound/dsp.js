...
ADSR.prototype.processOLDER = function(buffer) {
  for ( var i = 0; i < buffer.length; i++ ) {
    buffer[i] *= this.value();
...
ADSR.prototype.processOLD = function(buffer) {
	for ( var i = 0; i < buffer.length; i++ ) {
		//    buffer[i] *= this.value();

		if ( this.samplesProcessed <= this.attack ) {
			buffer[i] *= ( 0 + (1 - 0) * ((this.samplesProcessed - 0) / (this.attack - 0)));
		} else if ( this.samplesProcessed > this.attack && this.samplesProcessed <= this.decay ) {
			buffer[i] *= ( 1 + (this.sustainLevel - 1) * ((this.samplesProcessed - this.attack) / (this.decay - this.attack)));
		} else if ( this.samplesProcessed > this.decay && this.samplesProcessed <= this.sustain ) {
			buffer[i] *= ( this.sustainLevel);
		} else if ( this.samplesProcessed > this.sustain && this.samplesProcessed <= this.release ) {
			buffer[i] *= ( this.sustainLevel + (0 - this.sustainLevel) * ((this.samplesProcessed - this.sustain) / (this.release - this.sustain)));
		}
...
ADSR.prototype.processX = function(buffer, noteValue, start, end) {
	
	var j = 0;
	//add single note to samples
	for (var i = start; i < end; i++) {
		//    buffer[i] *= this.value();
		var f = Math.sin(noteValue * j) * .4;
		
		if ( j <= this.attack ) {
			buffer[i] += f*(j/this.attack);
		} else if ( j > this.attack && j <= this.decay ) {
			buffer[i] += f*( 1 + (this.sustainLevel - 1) * ((j - this.attack) / (this.decay - this.attack)));
		} else if ( j > this.decay && j <= this.sustain ) {
			buffer[i] += f*( this.sustainLevel);
		} else if ( j > this.sustain && j <= this.release ) {
			buffer[i] += f*( this.sustainLevel + (0 - this.sustainLevel) * ((j - this.sustain) / (this.release - this.sustain)));
		}
		j++;
	}
...
ADSR.prototype.getNoteValue = function(frequency) {
	return 2* Math.PI * frequency / this.sampleRate;
};


ADSR.prototype.process = function(buffer, noteValue, start, end, noteCache, noteCacheOffset) {
	var j = 0;
	//add single note to samples
	var volume = 0.4;
	for (var i = start; i < end; i++) {

		var f = Math.sin(noteValue * j) * volume;
		var v = 0;
		if ( j <= this.attack ) {
			v = f*(j/this.attack);
		} else if ( j > this.attack && j <= this.decay ) {
			v = f*( 1 + (this.sustainLevel - 1) * ((j - this.attack) / (this.decay - this.attack)));
		} else if ( j > this.decay && j <= this.sustain ) {
			v = f*( this.sustainLevel);
		} else if ( j > this.sustain && j <= this.release ) {
			v = f*( this.sustainLevel + 
					(0 - this.sustainLevel) * ((j - this.sustain) / (this.release - this.sustain)));
		}
		v = f;
		buffer[i] += v;
		noteCache[noteCacheOffset+j] = v;
		j++;
	}
...
ADSR.prototype.processBendX = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
	var j = 0;
	//add single note to samples
	var duration = (end-start);
	var volume = 1.0;
	var st = bendPoints[0];
	var interval = Math.round(duration*bendPoints[2]/12); // bendPoints[2] -> {2,3,4,6,12} (is always the divisor)
	var noteFreq = 440 * Math.pow(2,(noteValue - 12)/12);			
	consolewrp.log( "noteValue: " + noteValue );
	consolewrp.log( "noteFreq: " + noteFreq );
	var noteFactor = this.getNoteValue(noteFreq);
	consolewrp.log( "noteFactor: " + noteFactor );
	consolewrp.log( "interval: " + interval );
	var numDiffs = 1;
	var currNoteValue = noteValue;
	var midPoint = interval/2;

	for (var pbi = 2; pbi < bendPoints.length; pbi+=2) {
		var nextNoteValue = bendPoints[pbi+1];
		consolewrp.log( "nextNoteValue1: " + nextNoteValue );
		if( nextNoteValue != bendPoints[pbi-1]){
			numDiffs++;
			//var pitchDiff = Math.abs(nextNoteValue-bendPoints[pbi-1])/Math.pow(2,numDiffs);
			//var pitchDiff = Math.abs(nextNoteValue-bendPoints[pbi-1])/Math.pow(2,pbi);
			var pitchDiff = (nextNoteValue-bendPoints[pbi-1])/Math.pow(2,pbi/2+1);
			consolewrp.log( "pitchDiff: " + pitchDiff );
			currNoteValue += pitchDiff;
			consolewrp.log( "currNoteValue: " + currNoteValue );
			//var nextNoteFreq = 440 * Math.pow(2,(noteValue+pitchDiff - 12)/12);			
			var nextNoteFreq = 440 * Math.pow(2,(currNoteValue - 12)/12);			
			consolewrp.log( "nextNoteFreq: " + nextNoteFreq );
			var nextNoteFactor = this.getNoteValue(nextNoteFreq);
			consolewrp.log( "nextNoteFactor: " + nextNoteFactor );
			var inc = (nextNoteFactor-noteFactor)/interval;
			consolewrp.log( "inc: " + inc );
			consolewrp.log( "noteFactor: " + noteFactor );			
			var dec = inc/interval;
			i=0;
			consolewrp.log("start: " + (start+interval*(pbi-2)/2+i));
			for (var i = 0; i < interval; i++) {

				var f = Math.sin(noteFactor * j) * volume;
				noteFactor+=inc;
				...
				//buffer[start+interval*(pbi-2)/2+i] += v;

				buffer[start+interval*(pbi-2)/2+i] += f;
				//noteCache[noteCacheOffset+j] = v;
				j++;

			}
			consolewrp.log( "noteFactor: " + noteFactor );
			consolewrp.log("end: " + (start+interval*(pbi-2)/2+i));

		} else {

			//var nextNoteFreq = 440 * Math.pow(2,(noteValue+nextNoteValue/2 - 12)/12);			
			//var nextNoteFactor = this.getNoteValue(nextNoteFreq);
			...
			for (var i = 0; i < interval; i++) {
				...
				//var f = Math.sin(nextNoteFactor * j) * volume;
				var f = Math.sin(noteFactor * j) * volume;
				var v = 0;
				if ( j <= this.attack ) {
					v = f*(j/this.attack);
		...
		// now noteValue == nextNoteValue;

		//consolewrp.log( "nextNoteValue2: " + nextNoteValue );
		//consolewrp.log( "nnnnNoteValue2: " + noteValue );
		//break;
	}		
	return buffer;
};
     
ADSR.prototype.processBendZZ = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
	var j = 0;
	//add single note to samples
	var duration = (end-start);
	...
	consolewrp.log( "noteValue: " + noteValue );
	consolewrp.log( "noteFreq: " + noteFreq );
	var noteAngularFrequency = this.getNoteValue(noteFreq);
	consolewrp.log( "noteAngularFrequency: " + noteAngularFrequency );
	consolewrp.log( "interval: " + interval );
	var numDiffs = 1;
	var currNoteValue = noteValue;
	...
	for (var pbi = 2; pbi < bendPoints.length; pbi+=2) {
		...
		if( nextNoteValue != bendPoints[pbi-1]){
			...
			var nextNoteFreq = 440 * Math.pow(2,(currNoteValue - 12)/12);			
			consolewrp.log( "nextNoteFreq: " + nextNoteFreq );
			var nextNoteAngularFrequency = this.getNoteValue(nextNoteFreq);
			consolewrp.log( "nextNoteAngularFrequency: " + nextNoteAngularFrequency );
			inc = (nextNoteAngularFrequency-noteAngularFrequency)/interval;
			consolewrp.log( "inc: " + inc );
			break;
	...
	consolewrp.log( "nextNoteAngularFrequency: " + nextNoteAngularFrequency );
	var midPoint = (duration/2);
	//consolewrp.log( "midPoint: " + midPoint );
	var dec = inc/midPoint;
	consolewrp.log( "dec: " + dec );

	//var testPoint = Math.sin(noteAngularFrequency * midPoint) * volume;

	var testPoint = Math.sin(nextNoteAngularFrequency * midPoint);
	consolewrp.log( "testPoint : " + testPoint );
	consolewrp.log( "testPointX: " + (nextNoteAngularFrequency * midPoint) );
	var done = false;
	var freqq = Math.asin(testPoint);///midPoint;
	consolewrp.log( "freqq: " + freqq );

//x = sin(y);
//y = asin(x) + 2*k*p = p-asin(x)+2*k*p;
//asin(x) = y - 2kp
// 2kp = y - asin(x)

	for (var i = 0; i < duration; i++) {

		var f = Math.sin(noteAngularFrequency * j);// * volume;
		//(i > duration/2)? noteAngularFrequency-=inc: noteAngularFrequency+=inc;
		//(i > duration/2)? noteAngularFrequency-=0: 
		//if( i > midPoint) inc -= dec;
		if(Math.abs(f) >= 1.0 ) {
			//consolewrp.log( "f: " + f );
		}

		if(Math.abs(testPoint-f) < 0.00001) {
			consolewrp.log( "f: " + f );
			consolewrp.log( "i: " + i );

			freqq = Math.asin(f);
			var tkp = noteAngularFrequency * j - freqq;
			consolewrp.log( "tkp: " + tkp + " : " + tkp/(Math.PI*2) );
			consolewrp.log( "freqq: " + freqq );

			//done = true;
		}
		noteAngularFrequency+=inc;
		if(false && i > midPoint){
			inc -= dec/1.3;
		}
...
ADSR.prototype.processBend = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {

	var isUpDown=false;
	for (var pbi = 0; pbi < bendPoints.length; pbi+=2) {

	}

	if(bendPoints[1] == 0 && bendPoints[bendPoints.length-1] == 0) {
		this.processUpDownBend(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset);
	} else {
		this.processUpBend(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset);
	}

};


ADSR.prototype.processUpDownBend = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
	var j = 0;
...
ADSR.prototype.processUpBend = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
	var j = 0;
...
//ADSR.prototype.processUpDownBend = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
ADSR.prototype.processBendZNZ = function(buffer, noteValue, bendPoints, start, end, noteCache, noteCacheOffset) {
	var j = 0;
