...
*/
	function AudioDevice(readFn, channelCount, preBufferSize, sampleRate, postWriteCallBack){
		var	devices	= AudioDevice.devices,
			dev;
		for (dev in devices){
			if (devices.hasOwnProperty(dev) && devices[dev].enabled){
				try{
					return new devices[dev](readFn, channelCount, preBufferSize, sampleRate, postWriteCallBack);
				} catch(e1){
				consolewrp.log( "e1: " + e1 );
				};
			}
		}
	...
	function mozAudioDevice(readFn, channelCount, preBufferSize, sampleRate, postWriteCallBack){
		...
		consolewrp.log( "preBufferSize: " + preBufferSize );
		var	self			= this,
		...
		function bufferFill(){
			if (tail){
				...
				if(postWriteCallBack){postWriteCallBack(audioDevice.mozCurrentSampleOffset()-preBufferSize);}
			...
			//consolewrp.log( "currentPosition: " + currentPosition );
			//consolewrp.log( "available: " + available );
			if (available > 0){
	...
	function webkitAudioDevice(readFn, channelCount, preBufferSize, sampleRate, postWriteCallBack){
		sampleRate	= allowedSampleRates[sampleRate] ? sampleRate : 44100;
		...
		function bufferFill(e){
			...
			//consolewrp.log( "currentTime: " + context.currentTime );
			

			// context.currentTime is seconds needs conversion!!
			if(postWriteCallBack){postWriteCallBack(context.currentTime-preBufferSize);}
			self.writeBuffers(soundData);
			self.recordData(soundData);
	...
	function dummyAudioDevice(readFn, channelCount, preBufferSize, sampleRate, postWriteCallBack){
		sampleRate	= allowedSampleRates[sampleRate] ? sampleRate : 44100;
